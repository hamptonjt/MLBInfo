...
module.exports = function (context, cb) {
  ...
  let tweet = '';
  let url = 'http://www.nationalpastime.com/site/index.php?action=baseball_team_search&baseball_team=New+York+Yankees';
  ...
  getData().then((data) =>  {
    for (let factoid of data) {
      ...
      sendTweet(tweet, in_reply_to).then(sentTweet => {
        ...
        if (gt) {
          tweet = '@TodayYankees ' + factoid.fact.substr(spaceIndex +1);
          //console.log(tweet);
